...
void calcReg1() {
    std::ifstream in("csReg1_Corrected.out");
    assert(in.is_open());
    ...
    std::ifstream inCS("spectrum_reg1.out");
    assert(inCS.is_open());
    ...
    FILE* file = fopen("azure_reg1.out","w");
    ...
    for(Int_t i = 0; i < reg3CMEnergy_.size(); i++) {
        ...
        double crossSectionFactor = pow(sin(labAngleReg*M_PI/180.)/sin(cmAngleLight*M_PI/180.), 2.0)*cos((cmAngleLight - labAngleReg)*M_PI/180.);
        double crossSectionLab = cs/crossSectionFactor;
        double crossSectionLabErr = csErr/crossSectionFactor;

        printf("%f %f %f %f %f\n", energyLight, labAngleReg, crossSectionLab, crossSectionLabErr, crossSectionFactor);
