...
void Spectra::Loop() {
  ...
  for(long jentry = 0; jentry < nentries; jentry++) {
    ...
    for(auto csi : csiDetect_) {
      ...
      for(auto &det : protonDet_) {
        ...
        if(det.det == 2 || det.det == 3 || det.det == 6 || det.det == 7) continue;
    ...
    for(auto det : protonDet_) {
      ...
      if(!dEEForwardCut[det.det]->IsInside(det.totalEnergy, dERegion)) continue;
  ...
  s1->Scale(1./numberB8);
  s1->Write();
  WriteSpectrumToFile(s1, 1);

//  DivideTargetThickness(s3);
//  SolidAngle(s3, 3);
//  s3->Scale(1./numberB8);
//  s3->Write();
//  WriteSpectrumToFile(s3, 3);
...
bool Spectra::AnalysisForwardCentral(std::vector<mmCenter> centerMatched_, std::vector<mmTrack> centerBeamTotal_,
  ...
  // if(centerBeamTotal_.empty()) return false;

  if(centerBeamTotal_.size() > 5) {
    int maxPeakRow;
    ...
    if(maxPeakMap.find(maxPeakRow) != maxPeakMap.end()) {
      vertex = maxPeakMap[maxPeakRow];
    }
    else {
      vertex = maxPeakRow*1.75;
      // std::cout << "Can't find max: " << maxPeakRow << " Energy: " << totalEnergy << std::endl;
    }
  }

  hVertexSiETotalRegion1->Fill(totalEnergy, vertex);
...
//  angle = 0.;
  ...
  if(pathLength > 600) pathLength = 600.;

  double protonEnergy = totalEnergy/1000.;
  if(protonEnergy > 7.4 && protonEnergy < 10.4) { // was 9.7
    protonEnergy = rng->Uniform(7.4, 10.4);
  }

  protonEnergy = protonMethane->AddBack(protonEnergy, pathLength);
...
bool Spectra::AnalysisForwardSide(std::vector<mmCenter> centerMatched_, std::vector<mmTrack> centerBeamTotal_,
  ...
  if(pathLength < 0) pathLength = 200.;
  if(pathLength > 550) pathLength = 550.;

  double combinedEnergy = totalEnergy/1000.;
  if(combinedEnergy > 10.5 && combinedEnergy < 13) {
    combinedEnergy = rng->Uniform(10.5, 13);
  }

  double protonEnergy = protonMethane->AddBack(combinedEnergy, pathLength);
  double beamEnergy = protonEnergy*(m1 + m2)*(m1 + m2)/(4.*m1*m2*cosAngle*cosAngle);
...
void Spectra::ReadSolidAngle() {
  ...
  std::ifstream inReg1("csReg1_CorrectedPads.out");
...
void Spectra::WriteSpectrumToFile(TH1F *f, int region) {
  ...
  for(int i = 1; i <= i_size - 1; i++) {
    double binCenter = x_axis->GetBinCenter(i);
    double binContent = f->GetBinContent(i);
    if(binContent < 0.0001) continue;
    double binError = f->GetBinError(i);
    double cmAngle;
    if(region == 1) {
      cmAngle = reg1CMAngle(binCenter);
    }
    else if(region == 3) {
      cmAngle = reg3CMAngle(binCenter);
    }
